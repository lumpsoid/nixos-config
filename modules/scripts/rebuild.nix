...
in {
  options = {
    scripts.rebuild-nix.enable =
      lib.mkEnableOption "enable bulding 'rebuild-nix' script";
  };

  config = lib.mkIf config.scripts.rebuild-nix.enable {
    home.packages = [
      (
        pkgs.writeShellApplication
        {
          ...
          # you can add environmental variables for this script
          #runtimeEnv = [];
